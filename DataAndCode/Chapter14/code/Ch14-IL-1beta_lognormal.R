...
inits = function() {
	list(beta1=rnorm(1, 0, 1), sig=runif(1, 0.5, 1.5), sigg=runif(1, 0.5, 1.5))
}

jags_model <- jags.model(file = "Ch14-IL-1beta_lognormal.txt", data = data,
...
### Now stan
stan_fit <- stan(file = 'Ch14-IL-1beta_lognormal.stan', data = data, chains = 3, iter=8000, warmup=5000,
                                  init = inits)
